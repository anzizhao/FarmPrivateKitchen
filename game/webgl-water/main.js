...
window.onload = function() {
    ...
    document.getElementById('pausedButton').onclick = pausedOp;
    document.getElementById('useSpherePhysicsButton').onclick = useSpherePhysicsOp;
    document.getElementById('drawButton').onclick = drawOp;

    //var ratio = window.devicePixelRatio || 1;
    var ratio = 0.7;

    function onresize() {
        var width = innerWidth - gap;
        var height = innerHeight- gap;
    ...
    function downOptouch(e) {
        oldX = e.targetTouches[0].pageX;
        oldY = e.targetTouches[0].pageY;
        var pageX = e.targetTouches[0].pageX;
        var pageY = e.targetTouches[0].pageY;
        var tracer = new GL.Raytracer();
        var ray = tracer.getRayForPixel(pageX * ratio, pageY * ratio);
        var pointOnPlane = tracer.eye.add(ray.multiply(-tracer.eye.y / ray.y));
        var sphereHitTest = GL.Raytracer.hitTestSphere(tracer.eye, ray, center, radius);
        if (sphereHitTest) {
            mode = MODE_MOVE_SPHERE;
    ...
    document.onmousedown = downOp;
    document.addEventListener('touchstart', downOptouch);

    function upOp(e) {
        mode = -1;
    };

    document.onmouseup = upOp;
    document.addEventListener('touchend', upOp);
    ...
    document.addEventListener('touchmove', moveOptouch);
